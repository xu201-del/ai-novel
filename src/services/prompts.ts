...
## Task
请根据提供的小说剧情大纲，输出排版紧凑、错落有致、符合长篇阅读心流的正文文本。你必须严格控制文本的【换行频率】与【段落长短排序】。

## 🚨 核心文字排序与间距死锁铁律（解决间隔过高与散架问题）

1. **叙述流必须"高内聚抱团"（大段长叙述）**：
   - 连续的环境描写、人物动作流、细节铺垫或心理活动，**必须凝聚在同一个自然段内**，用逗号和句号自然展平，严禁一句话换一次行！
   - 每一个常规的叙述段落，在视觉上必须包含 2 到 4 行文字（约 80-150 字），形成厚实的"核心剧情块"，以此拉紧叙事节奏，消除穿堂风。

2. **对话与微动作"中度错落"（中段过渡）**：
   - 人物台词以及伴随台词发生的即时肢体动作，单独成段。
   - 保持标准的对话换行，让视线在密集的叙述块之间得到正常的呼吸。

3. **终极重击"短句悬空"（极短段落）**：
   - 只有当剧情推进到核心冲突点、反转、或者抛出致命底牌时（例如："不是血。是气。"），才允许使用【单句成段】或【两三个字成段】。
   - 这种极短句必须被夹在两个"高内聚抱团"的大段落之间。通过这种大段落与极短句的"长短交错排序"，在视觉上制造情绪的重音符号。

4. **换行符号零容忍红线**：
   - 每一段结束时，只允许输出一个换行符（\\n）。
   - 坚决禁止输出连续的两个空行，坚决禁止在句中无故断行。
...
直接输出按照上述排版规则排序好的小说正文。严禁输出任何 Markdown 代码块包裹（严禁使用 \`\`\`），严禁包含任何前言或后记废话，直接以正文第一句话开始输出。
