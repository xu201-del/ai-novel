...
export function buildChapterBodyPrompt(params: {
  ...
  return `# Role: 顶级网文节奏总监 (DeepSeek 专属字数合并引擎)

## Task
请根据大纲编写正文。你必须输出视觉密度极高、坚决不松散、段落内连续平铺的小说文本。

## 🚨 文本输出换行符死锁红线（彻底解决 DeepSeek 句句换行问题）

1. **短描述必须"一写到底"（严禁单句断行）**：
   - 类似"聚成一个人形。人形很高。高到八尺。"这种连续的短句，**必须全部用句号连接，写在同一行内**！
   - 你的输出格式必须是：【聚成一个人形。人形很高，高到八尺。八尺的人站在宫门口，穿着一件红衣，红得像血。】
   - 严禁写完几个字或一句话就触发回车。连续的动作、环境、心理描写，哪怕字数再多，也必须平铺成一个至少 150 字以上的长自然段。

2. **对话与神态"禁止拆成两行"**：
   - 角色说话的内容，必须和他的即时神态、动作紧紧粘连在同一个自然段。
   - 错误示范（坚决禁止）：
     陈麻子盯着他，突然笑了。
     "原来是你。"
   - 正确示范（必须这样）：
...
3. **全局【换行符 \\n】总额锁死**：
   - 在整个章节的生成过程中，你只有在发生宏观的场景切换、或者换人说话时，才允许使用一个换行符（\\n）。
   - 坚决禁止输出连续的两个换行符（如 \\n\\n）。
...
直接输出按照上述规则极限压缩间距的小说正文。严禁输出任何 Markdown 代码块包裹（严禁使用 \`\`\`），严禁包含任何前言、后记或格式废话，直接以正文第一句话开始输出。`;
}
