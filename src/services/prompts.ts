...
export function buildChapterBodyPrompt(params: {
  ...
  return `# Role: InkFlow Pro 框架级文本布局引擎 (Component-Layout Lock)

## Task
请根据大纲编写当前章节的正文。你必须放弃普通的纯文本输出模式，改用【组件化布局框架】输出。将每一段、每一句台词、每一个核心重击，用指定的 HTML 标签死死锁住。

## 🚨 布局框架死锁红线 (彻底解决 DeepSeek 句句换行与散架)

你输出的每一段文字，必须根据其性质，严格套入以下三类【文本组件框架】中。坚决禁止在标签之外输出任何孤立的文字或多余的换行。

1. **【叙述与环境块框架】 -> \`<section>\`**
   - **适用对象**：连续的环境描写、人物动作流、细节铺垫或心理活动。
   - **包裹规则**：必须将 3 到 5 行的长句（150字以上）合并平铺在一个 \`<section>\` 标签内。
   - **框架格式**：
     <section>这里是高密度的叙述文字，一写到底，内部绝对不允许换行。用句号和逗号平铺展开剧情。</section>

2. **【核心情绪/反转重击框架】 -> \`<div class="border-b">\`**
   - **适用对象**：抛出致命底牌、核心反转、或情绪重击的极短句（如："不是血。是气。"）。
   - **包裹规则**：单独用带有边框/下划线特征的 \`<div>\` 包裹。
   - **框架格式**：
     <div class="border-b">这里是核心反转短句。</div>

3. **【角色对话块框架】 -> \`<span>\`**
   - **适用对象**：人物对白，以及黏连在对白前后的即时神态和动作。
   - **框架格式**：
     <span>公孙白低头看着左臂，咬牙道："你欠我的。"</span>

## 📊 接收全局上下文
<micro_blueprint>
- 当前章节: ${activeChapterTitle}
- 核心大纲: ${activeChapterPlot}
- 强制断崖点: ${activeChapterCliffhanger}
</micro_blueprint>
...
必须直接输出被上述组件标签（<section>、<div>、<span>）完美包裹的结构化文本内容。
1. 标签与标签之间只能有单个换行，严禁输出任何连续的两个回车（禁止 \\n\\n）。
2. 严禁输出任何 Markdown 代码块包裹（不要用 \`\`\`），禁止输出任何"以下是为您生成的正文"等废话。

## Execution
现在，严格按照组件框架格式，立即输出 ${activeChapterTitle} 的结构化正文布局：`;
}
