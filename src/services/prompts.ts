...
export function buildChapterBodyPrompt(params: {
  ...
  return `# Role: 工业级网文排版与视觉节奏控制器

## Task
请根据提供的小说剧情大纲，输出排版清爽、密度紧凑、符合长篇连续阅读心流的正文文本。你必须严格控制文本的【视觉间距】与【段落长短排序】。

## 🚨 文本输出与排版死锁规则（严控段落间隔，拒绝支离破碎）

1. **叙述流"超高内聚"（彻底消除大空行）**：
   - 所有的环境描写、人物动作、心理活动、或者细节铺垫，**必须全部连缀在同一个自然段内**。
   - 每一个常规自然段，字数必须达到 150 - 300 字。严禁写完一句话（甚至半句话）就换行。
   - 连续的动作流请用逗号或句号断句，让文本在视觉上连成一块扎实的"文字长城"，以此把网页上的多余垂直间距彻底挤压掉。

2. **对话与神态"一行式咬合"**：
   - 台词和动作紧密粘连。如果是连续的对话，请写成"角色A说：'……'，随即转过身去。"的紧凑结构。
   - 严禁把一句对白和它的动作描述拆成两段输出。

3. **核心反转短句（单句成段，但必须受控）**：
   - 只有当剧情推到极其致命的冲突点（例如："不是血。是气。"），才允许单独换行成段。
   - 这种极短句**在当前章节中出现的频率不得超过 3 次**。必须被前后两个高密度的长段落紧紧包夹，从而在视觉上制造瞬间的"情绪针刺感"，而不是整页散架。

4. **【核心红线】全局换行符严格清零**：
   - 在你生成的整篇文本中，**坚决禁止出现连续两个换行符（如 \\n\\n）**。
   - 段落与段落之间，只允许输出单个换行符（\\n）。
   - 严禁在段落内部的句子中间出现任何无故的截断换行。
...
直接输出按照上述规则排版好、间距高度压缩的小说正文。严禁输出任何 Markdown 代码块包裹（严禁使用 \`\`\` 符号），严禁包含任何前言、后记、标题或格式废话，直接以正文第一句话开始输出。`;
}
